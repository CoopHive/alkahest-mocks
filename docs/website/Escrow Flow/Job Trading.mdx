...
// Encode the raw decision context, then wrap it for the escrow
const decisionContext = encodeAbiParameters(parseAbiParameters("(string query)"), [
...
// Encode the raw decision context, then wrap it for the escrow
let decision_context = "capitalize hello world".abi_encode().into();
let demand_data = TrustedOracleArbiter::DemandData {
    oracle: charlie_address,
    data: decision_context.clone(),
};
...
// Bob requests arbitration from Charlie using the raw inner decision context
bytes memory decisionContext = abi.encode("capitalize hello world");
trustedOracleArbiter.requestArbitration(fulfillmentUid, charlie, decisionContext);
```
...
// Bob requests arbitration with the same raw context used by the escrow
...
bob_client
    .oracle()
    .request_arbitration(fulfillment_uid, charlie_address, decision_context)
    .await?;
```
...
# Bob requests arbitration with the same raw context used by the escrow
await bob_client.oracle.request_arbitration(fulfillment_uid, charlie, inner)
```
...
Charlie should listen for `ArbitrationRequested` events addressed to him. Each event carries the raw inner decision context (`TrustedOracleArbiter.DemandData.data`), which must also be supplied to `arbitrate`. The fulfillment attestation remains linked to its escrow through `refUID` so settlement cannot reuse a fulfillment for another escrow.
...
// Parse the raw decision context from the ArbitrationRequested event
string memory query = abi.decode(decisionContext, (string));
...
trustedOracleArbiter.arbitrate(fulfillmentUid, decisionContext, isValid);
```
...
let result = charlie_client
    ...
    .arbitrate_many_sync(
        |request| {
            let query = decode_query(&request.demand).ok()?;
...
def validate_job(attestation, demand):
    (query,) = abi_decode(["string"], demand)
...
// Bob waits for the exact fulfillment/context/oracle decision, then claims
const arbitration = await bobClient.arbiters.general.trustedOracle.waitForArbitration(
  fulfillment.uid,
  charlie,
  decisionContext,
  1000,
...
if arbitration.inner.data.decision {
    bob_client
        .erc20()
        .escrow()
        .default()
...
# Bob waits for the exact fulfillment/context/oracle decision, then claims
arbitration = await bob_client.oracle.wait_for_arbitration(
    fulfillment_uid,
    list(inner),
    charlie,
    None,
