...
impl Erc721Module {
    ...
    /// # Example
    /// ```rust,ignore
    /// client.erc721().util().approve(&token, ApprovalPurpose::Payment).await?;
    /// client.erc721().util().approve_all(token_contract, ApprovalPurpose::Escrow).await?;
    /// ```
...
mod tests {
    ...
    async fn test_decode_payment_obligation() -> eyre::Result<()> {
        ...
        let payment_data = crate::contracts::obligations::ERC721PaymentObligation::ObligationData {
            token: token_address,
            tokenId: id,
            payee,
        };
