...
contract OptimisticStringValidator is BaseObligation, BaseArbiter {
    ...
    function mediate(bytes32 validationUID) external returns (bool success_) {
        ...
        StringResultObligation.ObligationData memory resultData =
            abi.decode(resultAttestation.data, (StringResultObligation.ObligationData));
        success_ = _isCapitalized(data.query, resultData.result);

        if (!success_) {
            eas.revoke(
                RevocationRequest({
                    schema: ATTESTATION_SCHEMA, data: RevocationRequestData({uid: validationUID, value: 0})
                })
            );
        }
