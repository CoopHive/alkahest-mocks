...
contract ERC20PaymentObligation is BaseObligation, BaseArbiter {
        ...
        override
    {
        ...
        uint256 balanceBefore = IERC20(obligationData.token).balanceOf(obligationData.payee);

        bool success =
            IERC20(obligationData.token).trySafeTransferFrom(payer, obligationData.payee, obligationData.amount);

        // Check balance after transfer
        uint256 balanceAfter = IERC20(obligationData.token).balanceOf(obligationData.payee);

        // Verify the actual amount transferred
        if (!success || balanceAfter < balanceBefore + obligationData.amount) {
            revert ERC20TransferFailed(obligationData.token, payer, obligationData.payee, obligationData.amount);
        }
    }
        ...
        returns (bool)
    {
        ...
        return
            payment.token == demandData.token && payment.amount >= demandData.amount
