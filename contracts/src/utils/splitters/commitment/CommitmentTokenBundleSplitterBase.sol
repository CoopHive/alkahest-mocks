...
/// @title CommitmentTokenBundleSplitterBase
/// @notice Shared collection and distribution logic for commitment token-bundle splitters.
...
abstract contract CommitmentTokenBundleSplitterBase is CommitmentBaseSplitter, ERC1155Holder {
    ...
    event ArbitrationMade(bytes32 indexed decisionKey, bytes32 indexed intentHash, address indexed oracle);
    ...
    constructor(IEAS _eas, IEscrow _escrowObligation) CommitmentBaseSplitter(_eas, _escrowObligation) {}
    ...
    function arbitrate(bytes32 intentHash, bytes32 escrow, BundleSplit[] calldata splits) external virtual;
    ...
    function createFulfillmentAndCollectAndDistribute(
        ...
        (BundleSplit[] memory splits, EscrowObligationData memory escrowData) =
            _collectAndDecode(escrow, fulfillmentUid);
        address fulfiller = _recordedFulfiller(fulfillmentUid);
        for (uint256 s; s < splits.length; ++s) {
            address recipient = _resolveSentinel(splits[s].recipient, fulfillmentUid, fulfiller);
            _distributeAtomic(splits[s], escrowData, recipient);
        }
        emit EscrowCollectedAndDistributed(escrow, fulfillmentUid, fulfiller);
    }
    ...
    function getSplits(address oracle, bytes32 intentHash, bytes32 escrow)
        external
        view
